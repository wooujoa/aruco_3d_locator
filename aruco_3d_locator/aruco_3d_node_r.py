...
class Aruco3DNode(Node):
    ...
    def color_cb(self, msg: CompressedImage):
        ...
        if len(cv_depth.shape) < 2:
            self.get_logger().error(f"❌ Depth image shape invalid: shape={cv_depth.shape}")
            self._publish_debug(overlay, msg.header)
            return
        ...
        for idx, marker_id in enumerate(ids.flatten()):
            ...
            depth_m = self._median_depth_m(
                cv_depth, self.latest_depth_msg.encoding, u, v
            )
            ...
            try:
                ...
                out.header.frame_id = self.latest_depth_msg.header.frame_id
